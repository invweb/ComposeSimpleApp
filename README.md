...
Используется рхитектурный паттерн MVI [Model‑View‑Intent].

Ключевые свойства StateFlow
Всегда имеет текущее значение. В отличие от обычMVI [Model‑View‑Intent].
ного Flow, у StateFlow всегда есть «последнее известное значение», которое новый подписчик получает сразу при подписке.
